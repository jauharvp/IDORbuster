...
use crate::{DefaultHttpSender, HttpRequest, HttpResponse, HttpSender, REQUEST_COUNTER};
...
pub fn perform_login(
    ...
    let content = fs::read_to_string(credentials_file).map_err(|e| {
    ...
    let creds_map: HashMap<String, LoginCredentials> =
        if credentials_file.ends_with(".yaml") || credentials_file.ends_with(".yml") {
    ...
    let token = runtime.block_on(async {
        ...
        // Send login request
        let response = client
            .post(format!("{}{}", credentials.host, credentials.endpoint))
            .json(&login_body)
            .send()
            .await
            .map_err(|e| {
        ...
        // Parse response
        let login_response: serde_json::Value = response.json().await.map_err(|e| {
            Box::new(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
        ...
        // Extract token - adjust this based on the actual response format
        let token = if let Some(token_value) = login_response.get("token") {
            token_value.as_str().unwrap_or("").to_string()
        } else if let Some(auth_token) = login_response.get("authentication") {
            auth_token
                .get("token")
                .and_then(|t| t.as_str())
                .unwrap_or("")
        ...
        let token_type = if login_response.get("token_type").is_some() {
            login_response
                .get("token_type")
                .and_then(|t| t.as_str())
                .unwrap_or("Bearer")
        ...
        match sender.send_request(&request, None, verbose) {
            Ok(response) => {
                ...
                let response_filename =
                    if token.user_type == "admin" || !token.user_type.contains("user") {
...
pub fn compare_responses(
            ...
            if impersonation_resp.status_code == 200 || impersonation_resp.status_code == 201 {
                ...
                if orig_resp.status_code == 200 || orig_resp.status_code == 201 {
                    ...
                    println!(
                        ...
                        orig_resp.status_code.to_string().bright_green()
                    );
                    println!(
                        "  Impersonation response code: {}",
                        impersonation_resp.status_code.to_string().bright_green()
                    );
                    println!(
                        "  Original response file: {}",
                        orig_path.display().to_string().bright_blue()
                    );
                    println!(
                        "  Impersonation response file: {}",
                        impersonation_path.display().to_string().bright_blue()
                    );
    ...
    if found_vulnerabilities == 0 {
        ...
        println!(
            "\n⚠️ {} potential IDOR vulnerabilities found",
            found_vulnerabilities.to_string().bright_red()
        );
    }
...
pub fn process_idor_tests(
    ...
    println!(
        "\n🔍 Making requests with original user token ({})",
        original_user.bright_yellow()
    );
    ...
    let original_responses =
        make_requests_with_token(&json_files, &original_token, &sender, verbose)?;
    ...
    println!(
        "\n🕵️ Making requests with impersonation user token ({})",
        impersonation_user.bright_yellow()
    );
    let impersonation_responses =
        make_requests_with_token(&json_files, &impersonation_token, &sender, verbose)?;
